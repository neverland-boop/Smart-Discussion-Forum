...
<div class="h-full w-full bg-slate-900 flex overflow-hidden" x-data="{ mobileNavOpen: false }">
    ...
    <div :class="mobileNavOpen ? 'translate-x-0' : '-translate-x-full'"
         class="fixed inset-y-0 left-0 z-50 w-80 transform transition-transform duration-200 ease-out lg:static lg:translate-x-0 lg:shadow-none lg:z-auto bg-slate-800 border-r border-slate-700 flex flex-col flex-shrink-0 lg:transition-all lg:duration-300 {{ $showLeftSidebar ? 'lg:w-80' : 'lg:w-16' }}">
        ...
        <div class="h-16 p-4 border-b border-slate-700 bg-slate-800 flex items-center {{ $showLeftSidebar ? 'justify-between' : 'justify-center' }}">
            @if($showLeftSidebar)
                ...
                <div class="flex items-center gap-1">
                    ...
                    <button wire:click="$toggle('showLeftSidebar')" class="hidden lg:inline-flex text-slate-400 hover:text-white p-1.5 rounded-md hover:bg-slate-700 transition" title="Toggle Sidebar">
                    ...
                    <!-- Mobile close -->
                    <button @click="mobileNavOpen = false" class="lg:hidden text-slate-400 hover:text-white p-1.5 rounded-md hover:bg-slate-700 transition" title="Close">
    ...
    <div class="flex-1 flex flex-col bg-slate-900 relative min-w-0" x-data="{ showMembers: false }">
        ...
        <!-- Mobile top bar -->
        <div class="h-14 flex items-center gap-3 px-4 border-b border-slate-700 bg-slate-800 lg:hidden shrink-0">
            <button @click="mobileNavOpen = true" type="button" class="p-2 -ml-2 rounded-md text-slate-300 hover:text-white hover:bg-slate-700 transition">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
                </svg>
            </button>
            <span class="text-white font-semibold text-sm truncate">{{ $topicModel->title ?? 'Discussions' }}</span>
        </div>
        ...
        @if($activeTopic && $topicModel)
            ...
            @if($isAuthorized)
                ...
                <div class="h-16 border-b border-slate-700 bg-slate-800 flex items-center px-4 sm:px-6 flex-shrink-0 justify-between">
                    <div>
                                ...
                                <div class="flex items-center gap-2 {{ $message['is_mine'] ? 'flex-row-reverse' : 'flex-row' }}">
                                    <div class="p-3 rounded-2xl shadow-sm {{ $message['is_mine'] ? 'bg-green-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 border border-slate-700 rounded-tl-none' }}">
                                        @if(!empty($message['attachment']))
                                            <div class="mb-2">
                                                <a href="{{ asset('storage/' . $message['attachment']) }}" target="_blank" class="flex items-center gap-2 text-sm bg-black/20 p-2 rounded-lg hover:bg-black/30 transition w-fit">
                ...
                <div class="p-4 bg-slate-800 border-t border-slate-700 shrink-0">
                    ...
                    @if($attachment)
                        <div class="mb-3 flex items-center gap-2 text-sm text-blue-400 bg-blue-900/20 p-2.5 rounded-lg border border-blue-500/30 w-fit max-w-full">
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                            <span class="truncate">{{ $attachment->getClientOriginalName() }}</span>
                            <button type="button" wire:click="$set('attachment', null)" class="text-slate-400 hover:text-red-400 ml-2 transition">
                    ...
                    <form wire:submit="sendMessage" class="flex items-center gap-3 max-w-4xl mx-auto" wire:key="chat-input-form">
                        ...
                        <label for="file-upload" class="cursor-pointer text-slate-400 hover:text-green-400 p-2 rounded-full hover:bg-slate-700 transition" title="Attach File">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                        </label>

                        <div wire:loading wire:target="attachment" class="text-green-500">
                        ...
                        <input 
                            ...
                            class="flex-1 bg-slate-900 border border-slate-700 text-white rounded-full px-5 py-3 focus:outline-none focus:border-green-500 shadow-inner placeholder-slate-500 transition-colors"
                            autocomplete="off"
                        ...
                        <button 
                            type="submit" 
                            class="bg-green-600 hover:bg-green-500 text-white p-3 rounded-full transition shadow-md focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center h-12 w-12 shrink-0"
                            wire:loading.attr="disabled"
                        >
                    ...
                    @error('newMessage') <span class="text-red-400 text-xs ml-14 mt-1 block">{{ $message }}</span> @enderror
                    @error('attachment') <span class="text-red-400 text-xs ml-14 mt-1 block">{{ $message }}</span> @enderror
                </div>
                ...
                <div x-show="showMembers" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="absolute inset-y-0 right-0 w-full max-w-xs sm:w-72 sm:max-w-none bg-slate-800 border-l border-slate-700 shadow-2xl flex flex-col z-50" style="display: none;">
        ...
        @else
            <!-- ZERO-DATA STATE -->
            <div class="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 bg-slate-900/50">
                ...
                <h2 class="text-2xl sm:text-3xl font-bold text-white mb-2 text-center">Welcome to Discussions</h2>
    ...
    @if($showTopicModal)
    <div class="fixed inset-0 bg-slate-900 bg-opacity-75 flex items-center justify-center z-50 p-4">
        <div class="bg-slate-800 p-6 rounded-xl w-full max-w-96 shadow-2xl border border-slate-700">
